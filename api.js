...
export function connect(floorId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.onclose = (evt) => {
            console.log(
                `[WS] Disconnected (code=${evt.code}, reason=${evt.reason})`,
            );
            stopHeartbeat();
            ...
            if (!settled) {
                ...
                reject(
                    new Error(
                        `WebSocket closed before opening (code=${evt.code})`,
                    ),
                );
            }
...
export async function upsertEstablishment(pointId, fields) {
    ...
    if (fields.opening_hours)
        form.append("opening_hours", fields.opening_hours);
...
export async function upsertEstablishmentBanner(pointId, bannerFile) {
    ...
    const res = await fetch(
        `${API_BASE}/points/${pointId}/establishment/banner`,
        {
            method: "PUT",
            headers: { Authorization: `Bearer ${AUTH_TOKEN}` },
            body: form,
